...
export function replaceInFile(
	params: {
		...
		replacingStr: string;
		/** Default false */
		doEnableMultipleOccurenceToBePresentAndOnlyReplaceTheFirstOnce?: boolean;
	}
) {

	const { filePath, replacedStr, replacingStr, doEnableMultipleOccurenceToBePresentAndOnlyReplaceTheFirstOnce = false } = params;

	const rawYaml = fs.readFileSync(filePath).toString("utf8");

	if (!doEnableMultipleOccurenceToBePresentAndOnlyReplaceTheFirstOnce) {
